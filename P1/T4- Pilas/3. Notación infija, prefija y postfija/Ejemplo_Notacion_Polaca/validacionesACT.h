...
std::string ingresarExpresion() {
    ...
    char elementoAnteriorAnterior = '\0';
    ...
    while (true) {
        ...
        elementoAnteriorAnterior = (i > 0) ? entrada[i - 2] : '\0';
        ...
        if (tecla == '\r' && i > 0 && !esOperador(elementoAnterior) && elementoAnterior != '.') {
            ...
            if (tecla == '(' && elementoAnterior != '.' && (esOperador(elementoAnterior) || elementoAnterior == '(' || i == 0)) {
                ...
                entrada[i++] = tecla;
                std::cout << tecla;
            } else if (tecla == ')' && elementoAnterior != '.' && elementoAnterior != '('
                                && !esOperador(elementoAnterior) && (isdigit(elementoAnterior) || elementoAnterior == ')' || elementoAnterior == 'i')) {
                parentesisCierre++;
        ...
        } else if (esOperador(tecla) && elementoAnterior != '.' && !esOperador(elementoAnterior) && elementoAnterior != '(' && i > 0 && contadorOp==false ) {
            entrada[i++] = tecla;
            std::cout << tecla;
            huboPunto = hayPunto;
            hayPunto = false;
        } else if (tecla == '-' && !esOperador(elementoAnterior) && elementoAnterior != '.' && elementoAnterior == '(' && elementoAnteriorAnterior != 'z') {
            entrada[i++] = tecla;
            std::cout << tecla;
            contadorOp = true;

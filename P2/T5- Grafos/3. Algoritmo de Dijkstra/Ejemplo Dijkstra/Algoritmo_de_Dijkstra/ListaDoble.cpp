...
template <typename T>
void ListaDoble<T>::eliminar_cola(){
    Nodo<T>* aux = cola;

    if(aux!=nullptr){

        if(aux->get_anterior() != nullptr){
            aux->get_anterior()->set_siguiente(nullptr);
            cola = aux->get_anterior();
        }
        else{
            cabeza = nullptr;
            cola = nullptr;
...
template <typename T>
bool ListaDoble<T>::buscar(T valor_busq){
    Nodo<T>* aux = cabeza;
    int pos = 0;
    bool encontrado;
    while(aux!=nullptr){
    ...
    encontrado = (aux!=nullptr) ? true : false;
    return encontrado;
}
